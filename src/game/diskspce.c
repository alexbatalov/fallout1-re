...
// 0x42BB30
int GetFreeDiskSpace(unsigned long* diskSpacePtr)
{
    char* path;
    ...
    *diskSpacePtr = 0;
    ...
    if (_getdiskfree(driveNumber, &df) == 0) {
        *diskSpacePtr = ((unsigned long)df.bytes_per_sector * (unsigned long)df.sectors_per_cluster * (unsigned long)df.avail_clusters) / 1024;
        return 0;
    }

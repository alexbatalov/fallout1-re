...
static_assert(sizeof(ScriptListExtent) == 0xD08, "wrong size");
...
int exec_script_proc(int sid, int action)
{
    ...
    if ((script->scr_flags & SCRIPT_FLAG_0x01) == 0) {
        char name[16];
        if (scr_list_str(script->scr_script_idx, name) == -1) {
            return -1;
        }
...
// 0x492FC4
static int scr_index_to_name(int scr_script_idx, char* name)
{
    int rc = -1;
    char path[MAX_PATH];
    DB_FILE* stream;
...
// 0x492FBC
int scr_list_str(int index, char* name)
{
    name[0] = '\0';
    return scr_index_to_name(index & 0xFFFFFF, name);
}
...
int scr_get_dialog_msg_file(int a1, MessageList** messageListPtr)
{
    ...
    if (messageList->entries_num == 0) {
        char scriptName[16];
        scr_list_str(messageListIndex, scriptName);
...
int scr_get_local_var(int sid, int variable, int* value)
{
    ...
    if (script->scr_num_local_vars > 0) {
        ...
        *value = map_get_local_var(script->scr_local_var_offset + variable);
    }

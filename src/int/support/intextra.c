...
static void op_reaction_roll(Program* program)
{
    ...
    for (int arg = 0; arg < 3; arg++) {
        ...
        if ((opcode[arg] & VALUE_TYPE_MASK) != VALUE_TYPE_INT) {
            interpretError("script error: %s: invalid arg %d to reaction", program->name, arg);
        }
    }

    interpretPushLong(program, reaction_roll(data[2], data[1], data[0]));
    interpretPushShort(program, VALUE_TYPE_INT);
}
...
static void op_reaction_influence(Program* program)
{
    ...
    interpretPushLong(program, reaction_influence(data[2], data[1], data[0]));
    interpretPushShort(program, VALUE_TYPE_INT);
}
...
static void op_local_var(Program* program)
{
    ...
    if ((opcode & VALUE_TYPE_MASK) != VALUE_TYPE_INT) {
        interpretError("script error: %s: invalid arg to op_local_var", program->name);
    }
...
static void op_turn_off_objs_in_area(Program* program)
{
    ...
    for (int arg = 0; arg < 4; arg++) {
        ...
        if ((opcode[arg] & VALUE_TYPE_MASK) != VALUE_TYPE_INT) {
            interpretError("script error: %s: invalid arg %d to turn_off_objs_in_area", program->name, arg);
        }
    }
...
static void op_turn_on_objs_in_area(Program* program)
{
    ...
    for (int arg = 0; arg < 4; arg++) {
        ...
        if ((opcode[arg] & VALUE_TYPE_MASK) != VALUE_TYPE_INT) {
            interpretError("script error: %s: invalid arg %d to turn_on_objs_in_area", program->name, arg);
        }
    }
...
static void op_barter_offer(Program* program)
{
    ...
    for (int arg = 0; arg < 3; arg++) {
        ...
        if ((opcode[arg] & VALUE_TYPE_MASK) != VALUE_TYPE_INT) {
            interpretError("script error: %s: invalid arg %d to barter_offer", program->name, arg);
        }
    }
}
...
static void op_barter_asking(Program* program)
{
    ...
    for (int arg = 0; arg < 3; arg++) {
        ...
        if ((opcode[arg] & VALUE_TYPE_MASK) != VALUE_TYPE_INT) {
            interpretError("script error: %s: invalid arg %d to barter_asking", program->name, arg);
        }
    }
}
...
static void op_metarule(Program* program)
{
    ...
    case METARULE_FIRST_RUN:
        interpretPushLong(program, (map_data.flags & MAP_SAVED) == 0);
        interpretPushShort(program, VALUE_TYPE_INT);
        break;
    case METARULE_ELEVATOR:
        scripts_request_elevator(param);
        interpretPushLong(program, 0);
        interpretPushShort(program, VALUE_TYPE_INT);
        break;
    case METARULE_PARTY_COUNT:
        interpretPushLong(program, getPartyMemberCount());
        interpretPushShort(program, VALUE_TYPE_INT);
        break;
    case METARULE_IS_LOADGAME:
        interpretPushLong(program, isLoadingGame());
        interpretPushShort(program, VALUE_TYPE_INT);
        break;
    default:
        interpretPushLong(program, 0);
...
static void op_play_gmovie(Program* program)
{
    ...
    unsigned short flags = game_movie_flags[data];
    if (data == MOVIE_VEXPLD || data == MOVIE_CATHEXP) {
        if (map_data.name[0] == '\0') {
            flags |= GAME_MOVIE_FADE_OUT;
        }
    }

    if (gmovie_play(data, flags) == -1) {
...
static void op_obj_is_open(Program* program)
{
    ...
    if ((opcode & VALUE_TYPE_MASK) != VALUE_TYPE_INT) {
        interpretError("script error: %s: invalid arg to obj_is_open", program->name);
    }
    ...
    interpretPushLong(program, isOpen);
    interpretPushShort(program, VALUE_TYPE_INT);
}
...
static void op_obj_name(Program* program)
{
    ...
    interpretPushLong(program, interpretAddString(program, strName));
    interpretPushShort(program, VALUE_TYPE_DYNAMIC_STRING);
}
...
static void op_get_pc_stat(Program* program)
{
    ...
    interpretPushLong(program, stat_pc_get(data));
    interpretPushShort(program, VALUE_TYPE_INT);
}
